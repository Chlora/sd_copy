...
#include "zookeper-interactions.h"

#define MAX_ARGS 7
int quitSignal = 0;

void cleanRlists() {
    int result1 = disconnectFromZookeeper();
    if (result1 != 0) {
        printf("Erro ao desconectar do Zookeeper.\n");
    }

    int result2 = rlist_disconnect(rlistHead);
    if (result2 != 0) {
        printf("Erro ao desconectar do servidor head.\n");
    }

    int result3 = rlist_disconnect(rlistTail);
    if (result3 != 0) {
        printf("Erro ao desconectar do servidor tail.\n");
    }
}

int assignZookeperServers(char* ip) {
    int result = connectToZookeper(ip);
...
int commandAdd(int argc, char **argv) {
    ...
    int result = rlist_add(rlistHead, car);
    data_destroy(car);
...
int commandGetByMarca(int argc, char **argv) {
    ...
    struct data_t *result = rlist_get_by_marca(rlistTail, marca_enum);
    if (result == NULL) {
...
int commandGetByYear(int argc, char **argv) {
    ...
    struct data_t **result = rlist_get_by_year(rlistTail, year_int);
...
int commandGetModelList(int argc, char **argv) {
    ...
    char **arr = rlist_get_model_list(rlistTail);
    if (arr == NULL) {
...
int commandGetListOrderedByYear(int argc, char **argv) {
    ...
    int result = rlist_order_by_year(rlistHead);
    if (result == -1) {
...
int commandRemove(int argc, char **argv) {
    ...
    int result = rlist_remove_by_model(rlistHead, argv[0]);
    if (result == -1) {
...
int commandSize(int argc, char **argv) {
    ...
    int result = rlist_size(rlistTail);
    if (result == -1) {
...
static int dispatch_command(int argc, char **argv) {
    ...
    printf("Corra 'help' para ver os comandos disponíveis\n");
    return -1;
}
...
int main(int argc, char *argv[]) {
    ...
    //conectar
    int result = assignZookeperServers(argv[1]);
